...
public class RegisterActivity extends AppCompatActivity {
    ...
    public void register(View view) {
        ...
        if ((!name.isEmpty()) && (!aadhar.isEmpty()) && (aadhar.length() > 11) && (!econsumer.isEmpty()) && (econsumer.length() > 9) && (location != null)) {
            ...
            databaseReference=FirebaseDatabase.getInstance().getReference("SENSORS").child("LDR-PDR");
            databaseReference.child("DOOR STATUS").setValue("door_false");
            databaseReference=FirebaseDatabase.getInstance().getReference("SENSORS").child("LDR-PDR");
            databaseReference.child("LDR STATUS").setValue("ldr_false");
            databaseReference=FirebaseDatabase.getInstance().getReference("SENSORS").child("LDR-PDR");
            databaseReference.child("PIR STATUS").setValue("pir_false");
            databaseReference=FirebaseDatabase.getInstance().getReference("SENSORS").child("LDR-PDR");
            databaseReference.child("IDENTIFIED FACE").setValue("Intruder");
